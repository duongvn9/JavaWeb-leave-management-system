...
<head>
    <title>Báo cáo tổng quan agenda nghỉ</title>
    <style>
        body {
            background: #f8fafc;
        }
    ...
    <script>
        function parseDMY(str) {
            ...
            // Nếu là yyyy-MM-dd (khi submit), cũng parse được
            parts = str.split('-');
            if (parts.length === 3) {
        ...
        window.addEventListener('DOMContentLoaded', function () {
            flatpickr("#from", {
                dateFormat: "d/m/Y",
                allowInput: true,
                defaultDate: document.getElementById('from').value ? document.getElementById('from')
                    .value.split('-').reverse().join('/') : null
            });
            flatpickr("#to", {
                dateFormat: "d/m/Y",
                allowInput: true,
                defaultDate: document.getElementById('to').value ? document.getElementById('to').value
                    .split('-').reverse().join('/') : null
            });
            // Xử lý khi submit form: chuyển input về yyyy-MM-dd
            document.querySelector('.date-form').addEventListener('submit', function (e) {
                ...
                if (from.value) {
                    ...
                    if (parts.length === 3) {
                        from.value = parts[2] + '-' + parts[1].padStart(2, '0') + '-' + parts[0]
                            .padStart(2, '0');
                    }
                }
                if (to.value) {
                    ...
                    if (parts.length === 3) {
                        to.value = parts[2] + '-' + parts[1].padStart(2, '0') + '-' + parts[0].padStart(
                            2, '0');
                    }
                }
            });
