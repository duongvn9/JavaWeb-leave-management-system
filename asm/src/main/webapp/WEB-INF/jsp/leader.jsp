...
<head>
    ...
    <style>
        ...
        .leader-wrapper { display: flex; min-height: 100vh; }
        .sidebar {
            width: 250px;
...
<div class="leader-wrapper">
    ...
    <nav class="sidebar" id="sidebar">
        <div class="sidebar-title">
            <a href="#" id="sidebar-home-btn" style="color:inherit;text-decoration:none;display:flex;align-items:center;gap:0.7rem;font-weight:700;font-size:1.3rem;">
                <i class="fa-solid fa-user-tie"></i> Trưởng phòng
            </a>
        </div>
        <a href="#" class="nav-link" id="menu-leave-review">
            <i class="fa-solid fa-clipboard-check"></i> Duyệt đơn nhân viên
        </a>
    ...
    <div class="system-content" id="systemContent">
        <div class="header-bar">
            <h1 class="header-title" id="page-title">Trang trưởng phòng</h1>
            <div class="user-info">
                <span><i class="fa-solid fa-user-tie"></i> ${sessionScope.user.fullName}</span>
            ...
            <div style="flex:1;display:flex;align-items:center;justify-content:center;min-height:60vh;padding:2rem;">
                <div style="text-align:center;max-width:600px;margin:0 auto;">
                    <div style="background: linear-gradient(135deg, #4285F4 0%, #34a853 100%); width: 90px; height: 90px; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 1.5rem auto; box-shadow: 0 8px 32px rgba(66,133,244,0.15);">
                        <i class="fa-solid fa-user-tie" style="font-size:2rem;color:#fff;"></i>
                    </div>
                    <h2 style="font-size:2rem;font-weight:800;margin-bottom:1rem;color:#232946;">Chào mừng, ${sessionScope.user.fullName}!</h2>
                    <div style="font-size:1.1rem;color:#555;line-height:1.7;">Bạn có thể duyệt đơn, tạo đơn hoặc xem danh sách nhân viên phòng ban ở menu bên trái.</div>
                </div>
            </div>
        </div>
        ...
        <div id="tab-about" class="tab-pane">
            <div style="flex:1;display:flex;align-items:center;justify-content:center;min-height:60vh;">
                <div style="text-align:center;max-width:500px;margin:0 auto;">
                    ...
                    <a href="https://github.com/duongvn9/JavaWeb-leave-management-system" target="_blank" class="btn btn-dark me-2" style="text-decoration:none; font-size:1.25rem; padding:0.9rem 2.2rem; border-radius:10px; display:inline-flex; align-items:center; gap:0.7rem;"><i class="fab fa-github" style="font-size:1.6rem;"></i> GitHub</a>
                    <a href="https://www.facebook.com/ngocduonggvu" target="_blank" class="btn btn-primary" style="text-decoration:none; font-size:1.25rem; padding:0.9rem 2.2rem; border-radius:10px; display:inline-flex; align-items:center; gap:0.7rem;"><i class="fab fa-facebook" style="font-size:1.6rem;"></i> Facebook</a>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    let sidebarCollapsed = localStorage.getItem('leaderSidebarCollapsed') === 'true';
    ...
    function toggleSidebar() {
        ...
        localStorage.setItem('leaderSidebarCollapsed', sidebarCollapsed);
    ...
    document.getElementById('sidebar-home-btn').onclick = function(e) {
        e.preventDefault();
        setActiveTab('home');
        document.getElementById('page-title').textContent = 'Trang trưởng phòng';
        localStorage.removeItem('leaderTab');
    };
    // Tab menu
    const menuItems = {
        'menu-leave-review': { tab: 'leave-review', title: 'Duyệt đơn nhân viên' },
    ...
    Object.keys(menuItems).forEach(menuId => {
        document.getElementById(menuId).onclick = function(e) {
            ...
            localStorage.setItem('leaderTab', item.tab);
        };
    });
    ...
    function restoreTabState() {
        const lastTab = localStorage.getItem('leaderTab');
    ...
    document.getElementById('logoutBtn').onclick = function(e) {
        ...
        document.querySelector('.leader-wrapper').classList.add('blur');
    };
    document.getElementById('cancelLogout').onclick = function() {
        document.getElementById('logoutModal').style.display = 'none';
        document.querySelector('.leader-wrapper').classList.remove('blur');
    };
