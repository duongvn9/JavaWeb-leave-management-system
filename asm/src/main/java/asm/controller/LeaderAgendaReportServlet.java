...
@WebServlet("/leader/agenda-report")
public class LeaderAgendaReportServlet extends HttpServlet {
    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        HttpSession session = request.getSession();
        User leader = (User) session.getAttribute("user");
        Integer deptId = leader != null ? leader.getDeptId() : null;
        ...
        List<User> users = deptId != null ? userDao.listByDepartment(deptId) : new ArrayList<>();
        ...
        // Lấy danh sách đơn nghỉ phép đã duyệt trong khoảng của phòng ban
        LeaveRequestService leaveService = new LeaveRequestService();
        List<LeaveRequest> leaves = leaveService.listByDepartmentAndStatus(deptId, false, "APPROVED");
        ...
        request.getRequestDispatcher("/WEB-INF/jsp/leader/agenda-report.jsp").forward(request, response);
    }
} 